using ComponentServiceLibrary.Dtos;
...
namespace Recruitment.Service.API.Persistence.Applications
{
	...
	) : ICandidateServices
	{
		...
		public async Task<(DataTableResponseDto<CandidateDto> DataTables, List<ProcessCountDto> processCounts)> RetrieveAllCandidateService(string? search, int start, int length, string draw, string sortColumnName, string sortDirection, int client, int clientGroup, int job, int qualification, int applicationProcess, int source)
		{
			JsonResult result;
			try
			{
				...
				var dataTableResult = DataTableComponent<CandidateDto>.Process(
					candidates,
					search,
					start,
					length,
				...
				return (dataTableResult, processCounts)!;
			...
			catch (Exception e)
			{
				...
				// Return an empty DataTableResponseDto with error info if needed
				return (new DataTableResponseDto<CandidateDto>
				{
					Draw = draw,
					RecordsTotal = 0,
					RecordsFiltered = 0,
