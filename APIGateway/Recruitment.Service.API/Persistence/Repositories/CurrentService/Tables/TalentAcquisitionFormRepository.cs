...
{
	public class TalentAcquisitionFormRepository(HrisDbContext context) : ITalentAcquisitionFormRepository
	{
		...
		{
			var tafList = context.TalentAcquisitionForms
				.Include(i => i.Job)
				.Include(i => i.Status)
				.Include(i => i.Reason)
				.Include(i => i.WorkArrangementDetail)
			...
			tafList = group == 0 ? tafList : tafList.Where(w => w.Department.DepartmentGroupId == group);
			tafList = department == 0 ? tafList : tafList.Where(w => w.DepartmentId == department);
			tafList = status == 0 ? tafList : tafList.Where(w => w.StatusId == status);
			tafList = reason == 0 ? tafList : tafList.Where(w => w.ReasonId == reason);

			var tafListDto = await tafList.Select(s => new TalentAcquisitionFormDashboardDto()
				{
					Id = s.Id,
