...
namespace Recruitment.Service.API.Persistence.Helpers
{
	...
	: IApplicationProcessHelper
	{
		...
		public async Task<JsonResult> SendEmailSalaryPackageApprovalTemplate(int candidateId, int templateId)
		{
			var emailTemplatePath = Path.Combine(_fileRootFolder ?? string.Empty, "document_template", "EmailTemplate.html");
			...
			var mailText = str.ReadToEnd();
			str.Close();
			var jobOfferInfo = await _uniForCurrentService.JobOfferRepository.RetrieveJobOfferInfo(candidateId);

			var candidate = await _uniForCurrentService.CandidateRepository.SelectCandidateInformation(candidateId);

			var hiringManager = await _uniForCurrentService.DepartmentRepository.RetrieveDepartmentIndividual(Convert.ToInt32(jobOfferInfo.ApproverId));

			{
				var emailContent = await _uniForCurrentService.EmailTemplateRepository.SelectEmailTemplateInformation(templateId);


				emailContent.EmailBody = emailContent.EmailBody.Replace("[hiring_manager_name]", $"{hiringManager.Name}");

				var encryptedId = EncryptProvider.Base64Encrypt(jobOfferInfo.CandidateId.ToString(), Encoding.Unicode);

				emailContent.EmailBody = emailContent.EmailBody.Replace("[approval_link]", "https://172.16.254.4/salarypackage/approval/" + $"{encryptedId}");




				mailText = mailText.Replace("[EmailTo]", "Hi " + $"{hiringManager.Name}");
				mailText = mailText.Replace("[Subject]", emailContent.Name.ToUpper());
				mailText = mailText.Replace("[EmailBody]", emailContent.EmailBody);
			...
			var result = new JsonResult(new { success = true, responseText = "Salary Package Email Sent" })
			{
				StatusCode = 200
			};
			return result;
		...
		public async Task<JsonResult> SendEmailSalaryPackageApprovedTemplate(int candidateId, int templateId)
		{
			var emailTemplatePath = Path.Combine(_fileRootFolder ?? string.Empty, "document_template", "EmailTemplate.html");
...

			{
				...
				emailContent.EmailBody = emailContent.EmailBody.Replace("[candidate_position]", $"{candidate.Job.Position}");
				...
				var emailDto = new EmailDto()
				{
					SentTo = ["Timothy.Diego@onecoredevit.com"],
					SendCc = emailContent.Cc.HasValue() ? emailContent.Cc.Split(",").ToList() : null,
					Body = mailText,
					Subject = emailContent.Subject
				};
		...
		public async Task<JsonResult> SendEmailSalaryPackageDeclinedTemplate(int candidateId, int templateId)
		{
			var emailTemplatePath = Path.Combine(_fileRootFolder ?? string.Empty, "document_template", "EmailTemplate.html");
...

			{
				...
				var emailDto = new EmailDto()
				{
					SentTo = ["TalentAcquisition1@onecoredevit.com"],
					SendCc = emailContent.Cc.HasValue() ? emailContent.Cc.Split(",").ToList() : null,
					Body = mailText,
					Subject = emailContent.Subject
				};
		...
		public async Task<JsonResult> SendEmailCompletedCoreInformationTemplate(int candidateId, int templateId)
		{
			var emailTemplatePath = Path.Combine(_fileRootFolder ?? string.Empty, "document_template", "EmailTemplate.html");
...

			{
				...
				emailContent.EmailBody = emailContent.EmailBody.Replace("[system_link]", "https://172.16.254.4/recruitment/viewonboarding?id=" + $"{candidateId}");
			...
			var result = new JsonResult(new { success = true, responseText = "Email Sent: Candidate Completed Core Information!" })
			{
				StatusCode = 200
			};
			return result;
		...
		public async Task<JsonResult> SendEmailReadyToStartToTeamTemplate(int candidateId, int templateId)
		{
			var emailTemplatePath = Path.Combine(_fileRootFolder ?? string.Empty, "document_template", "EmailTemplate.html");
			...
			var candidate = await _uniForCurrentService.CandidateRepository.SelectCandidateInformation(candidateId);
			var onboardingInfo = await _uniForCurrentService.OnboardingRepository.RetrieveOnboardingInformation(candidateId);
			{
				var emailContent = await _uniForCurrentService.EmailTemplateRepository.SelectEmailTemplateInformation(templateId);


				emailContent.EmailBody = emailContent.EmailBody.Replace("[candidate_full_name]", $"{onboardingInfo.FirstName}" + " " + $"{onboardingInfo.LastName}");

				emailContent.EmailBody = emailContent.EmailBody.Replace("[start_date]", $"{onboardingInfo.StartDate?.ToString("MMMM d, yyyy")}");
				emailContent.EmailBody = emailContent.EmailBody.Replace("[employee_id]", $"{onboardingInfo.TemporaryEmployeeId}");
				emailContent.EmailBody = emailContent.EmailBody.Replace("[position]", $"{candidate.Job.Position}");
				emailContent.EmailBody = emailContent.EmailBody.Replace("[mobile_number]", $"{onboardingInfo.MobileNo}");
		...
		public async Task<JsonResult> SendEmailReadyToStartToCandidateTemplate(int candidateId, int templateId)
		{
			var emailTemplatePath = Path.Combine(_fileRootFolder ?? string.Empty, "document_template", "EmailTemplate.html");
			...
			var onboardingInfo = await _uniForCurrentService.OnboardingRepository.RetrieveOnboardingInformation(candidateId);
			{
				...
				emailContent.EmailBody = emailContent.EmailBody.Replace("[candidate_first_name]", $"{onboardingInfo.FirstName}");

				emailContent.EmailBody = emailContent.EmailBody.Replace("[start_date]", $"{onboardingInfo.StartDate?.ToString("MMMM d, yyyy")}");
				emailContent.EmailBody = emailContent.EmailBody.Replace("[passcode]", "firstactivation");
				emailContent.EmailBody = emailContent.EmailBody.Replace("[system_link]", "system.onecoredevit.com/app");
