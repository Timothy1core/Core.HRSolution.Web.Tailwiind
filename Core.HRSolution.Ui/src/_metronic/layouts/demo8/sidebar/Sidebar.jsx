...
import { usePathname } from '@/_metronic/providers';

import { Sheet, SheetContent, SheetDescription, SheetHeader, SheetTitle } from '@/_metronic/components/ui/sheet';
const Sidebar = () => {
  ...
  const renderContent = () => {
    return <div className="grow lg:grow-0 lg:fixed top-0 bottom-0 z-20 flex flex-col items-stretch shrink-0 bg-[--tw-page-bg] dark:bg-[--tw-page-bg-dark]">
  ...
  if (desktopMode) {
    ...
    return <Sheet open={mobileSidebarOpen} onOpenChange={handleMobileSidebarClose}>
        <SheetContent className="border-0 p-0 w-[--tw-sidebar-width] flex items-stretch flex-col scrollable-y-auto" forceMount={true} side="left" close={false}>
