...
import { Menu, MenuArrow, MenuIcon, MenuItem, MenuLabel, MenuLink, MenuSub, MenuTitle, MenuToggle } from '@/_metronic/components/menu';
  ...
  const buildMenu = items => {
    return items.map((item, index) => {
      return <Fragment key={index}>
            ...
            <MenuLabel className="flex items-center justify-between">
              <MenuToggle className="cursor-pointer pb-2 pt-3 ps-[14.5px] rounded-md border border-transparent">
                <MenuArrow className="me-2.5">
                  ...
                  <KeenIcon icon="down" className="text-sm hidden menu-item-show:inline-flex text-gray-500" />
                </MenuArrow>
                <MenuTitle className="text-sm text-gray-800 font-medium">{item.title}</MenuTitle>
              </MenuToggle>
              {item.plus && <div className="btn btn-sm btn-icon btn-icon-sm text-gray-500 hover:text-gray-800">
                  <KeenIcon icon="plus" />
                </div>}
            </MenuLabel>
            <MenuSub className="menu-accordion gap-px">
              {item.children?.map((item2, index2) => {
              return <MenuItem key={index2} className={clsx(item2.active && 'active')}>
          ...
          {index !== items.length - 1 && <div className="border-b border-gray-300 mt-2 mb-1 mx-1.5"></div>}
        </Fragment>;
    });
  };
  return <Menu highlight={true} multipleExpand={true} className="flex flex-col w-full gap-1.5 px-3.5">
      {buildMenu(items)}
    </Menu>;
};
