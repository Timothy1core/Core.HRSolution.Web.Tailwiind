...
import { Menu, MenuArrow, MenuIcon, MenuItem, MenuLink, MenuSub, MenuTitle } from '@/_metronic/components/menu';
...
const SidebarMenuPrimary = () => {
  ...
  const buildMenuItemRoot = (item, index, level) => {
    if (item.children) {
      return <MenuItem key={index} toggle="accordion" trigger="click">
      ...
      return <MenuItem key={index}>
          <MenuLink path={item.path} className={clsx('gap-2.5 py-2 px-2.5 rounded-md border border-transparent menu-item-active:border-gray-200 menu-item-active:bg-light menu-link-hover:bg-light menu-link-hover:border-gray-200', subIndetion[level])}>
  ...
  const buildMenuItemChild = (item, index, level = 0) => {
    if (item.children) {
      return <MenuItem key={index} toggle="accordion" trigger="click" className={clsx(item.collapse && 'flex-col-reverse')}>
          <MenuLink className="py-2 px-2.5 rounded-md border border-transparent">
            {item.collapse ? <MenuTitle className="text-2sm text-gray-600 menu-item-here:text-gray-800 menu-item-show:text-gray-800 menu-link-hover:text-gray-800 dark:menu-item-here:text-gray-900 dark:menu-item-show:text-gray-900 dark:menu-link-hover:text-gray-900">
              ...
              </MenuTitle> : <MenuTitle className="text-2sm text-gray-800 menu-item-active:text-gray-900 menu-item-here:text-gray-900 menu-item-show:text-gray-900 menu-link-hover:text-gray-900">
                {item.title}
              </MenuTitle>}
      ...
      return <MenuItem key={index}>
          <MenuLink path={item.path} className="py-2 px-2.5 rounded-md border border-transparent menu-item-active:border-gray-200 menu-item-active:bg-light menu-link-hover:bg-light menu-link-hover:border-gray-200">
  ...
  const buildMenuArrow = () => {
    return <MenuArrow className="text-gray-600 menu-item-here:text-gray-800 menu-item-show:text-gray-800 menu-link-hover:text-gray-800">
  ...
  return <Menu highlight={true} multipleExpand={false} className="flex flex-col w-full gap-1.5 px-3.5">
