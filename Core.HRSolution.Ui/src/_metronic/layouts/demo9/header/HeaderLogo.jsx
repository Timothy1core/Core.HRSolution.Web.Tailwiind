...
import { Menu, MenuArrow, MenuIcon, MenuItem, MenuLink, MenuSub, MenuTitle, MenuToggle, KeenIcon } from '@/_metronic/components';
...
const HeaderLogo = () => {
  ...
  return <div className="flex items-center gap-1 lg:w-[400px] grow lg:grow-0">
      <button type="button" onClick={handleSidebarOpen} className="btn btn-icon btn-light btn-clear btn-sm -ms-2.5 lg:hidden">
        <KeenIcon icon="menu" />
      </button>

      <div className="flex items-center gap-2">
        <Link to="/">
          <img src={toAbsoluteUrl('/media/app/mini-logo-circle.svg')} className="dark:hidden min-h-[34px]" alt="logo" />
          <img src={toAbsoluteUrl('/media/app/mini-logo-circle-dark.svg')} className="hidden dark:inline-block min-h-[34px]" alt="logo" />
        </Link>
          ...
          modifiers: [{
            ...
            options: {
              ...
              <MenuSub className="menu-default w-48 py-2">
                {teams.map((team, index) => <MenuItem key={index}>
          ...
          modifiers: [{
            ...
            options: {
              ...
              <MenuSub className="menu-default w-48 py-2 scrollable-y max-h-[250px]">
                {months.map((item, index) => <MenuItem key={index}>
                    <MenuLink path="#">
