...
const SidebarMenuPrimary = () => {
  ...
  const buildMenuItemRoot = (item, index, level) => {
    if (item.children) {
      ...
      return <MenuItem key={index}>
          <MenuLink path={item.path} className={clsx('gap-2.5 py-2 px-2.5 rounded-md menu-item-active:bg-gray-100 menu-link-hover:bg-gray-100 !menu-item-here:bg-transparent', subIndetion[level])}>
  ...
  const buildMenuItemChild = (item, index, level = 0) => {
    if (item.children) {
      ...
      return <MenuItem key={index}>
          <MenuLink path={item.path} className="py-2 px-2.5 rounded-md menu-item-active:bg-gray-100 menu-link-hover:bg-gray-100">
  ...
  const buildMenuArrow = () => {
    return <MenuArrow className="text-gray-400 menu-item-here:text-gray-400 menu-item-show:text-gray-800 menu-link-hover:text-gray-800">
