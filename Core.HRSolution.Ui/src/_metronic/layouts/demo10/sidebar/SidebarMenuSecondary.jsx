...
import { Menu, MenuItem, MenuLink, MenuTitle, MenuIcon } from '@/_metronic/components/menu';
const items = [{
  logo: 'x-dark.svg',
  title: '@keenthemes',
...
const SidebarMenuSecondary = () => {
  return <div>
      ...
      <Menu className="flex flex-col w-full gap-1.5 px-3.5">
        {items.map((item, index) => <MenuItem key={index} className="menu-item">
            <MenuLink path={item.path} className="py-1 px-2.5">
              <MenuIcon>
                <span className="rounded-md flex items-center justify-center size-7 bg-coal-black menu-link-hover:border-none me-2">
                  <img src={toAbsoluteUrl(`/media/brand-logos/${item.logo}`)} className="size-3.5" alt={item.title} />
                </span>
              </MenuIcon>
