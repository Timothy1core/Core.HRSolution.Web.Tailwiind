import { Menu, MenuItem, MenuLink, MenuTitle, MenuToggle, KeenIcon, MenuSub, MenuIcon } from '@/_metronic/components';
...
const SidebarMenuDashboard = () => {
  ...
  return <div className="flex flex-col gap-7.5 px-2.5">
      ...
      <Menu highlight={true} className="flex-col gap-5">
        {menuItems.map((heading, index) => <div key={index} className="flex flex-col gap-px">
            <MenuItem>
              <div className="px-2 text-xs font-medium text-gray-600">{heading.title}</div>
            </MenuItem>

            {heading.children.map((item, index) => <MenuItem key={index} className={item.active ? 'active' : ''}>
                <MenuLink path={item.path} className="py-2 px-2.5 gap-2 rounded-md border border-transparent menu-item-active:border-gray-200 menu-item-active:bg-light menu-link-hover:bg-light menu-link-hover:border-gray-200">
                  <KeenIcon icon={item.icon} />
