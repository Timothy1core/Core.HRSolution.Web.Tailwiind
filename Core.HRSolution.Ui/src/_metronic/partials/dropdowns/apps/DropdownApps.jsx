...
import { MenuSub } from '@/_metronic/components';
...
const DropdownApps = () => {
  ...
  const buildHeader = () => {
    return <div className="flex items-center justify-between gap-2.5 text-2xs text-gray-600 font-medium px-5 py-3 border-b border-b-gray-200">
        <span>Apps</span>
        <span>Enabled</span>
      </div>;
  };
  const buildFooter = () => {
    return <div className="grid p-5 border-t border-t-gray-200">
  ...
  const buildList = items => {
    return <div className="flex flex-col scrollable-y-auto max-h-[400px] divide-y divide-gray-200">
        {items.map((item, index) => <div key={index} className="flex items-center justify-between flex-wrap gap-2 px-5 py-3.5">
            <div className="flex items-center flex-wrap gap-2">
              <div className="flex items-center justify-center shrink-0 rounded-full bg-gray-100 border border-gray-200 size-10">
                <img src={toAbsoluteUrl(`/media/brand-logos/${item.logo}`)} className="size-6" alt={item.title} />
              </div>
  ...
  return <MenuSub rootClassName="w-full max-w-[320px]" className="light:border-gray-300">
      {buildHeader()}

      <div className="scrollable-y-auto" style={{
      maxHeight: desktopMode ? '400px' : '75vh'
    }}>
        {buildList(items)}
      </div>
