...
import { Menu, MenuItem, MenuSub, MenuToggle } from '@/_metronic/components/menu';
import { Tab, TabPanel, Tabs, TabsList } from '@/_metronic/components/tabs';
import { DropdownCrud2 } from '@/_metronic/partials/dropdowns/general';
...
const DropdownNotifications = ({
  ...
  const buildHeader = () => {
    return <div className="flex items-center justify-between gap-2.5 text-sm text-gray-900 font-semibold px-5 py-2.5 border-b border-b-gray-200">
        Notifications
  ...
  const buildTabs = () => {
    return <Tabs defaultValue={1} className="">
  ...
  return <MenuSub rootClassName="w-full max-w-[460px]" className="light:border-gray-300">
      {buildHeader()}
      {buildTabs()}
    </MenuSub>;
};
