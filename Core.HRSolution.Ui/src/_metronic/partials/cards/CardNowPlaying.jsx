...
const CardNowPlaying = ({
  ...
  const renderItem = (statistic, index) => {
    return <div key={index} className="grid grid-cols-1 gap-1.5 text-center">
  ...
  return <div className="card shadow-none w-[280px] border-0">
      <img src={toAbsoluteUrl(`/media/images/600x600/${image}`)} className="rounded-t-xl max-w-[280px] shrink-0" alt="" />

      <div className="card-border card-rounded-b grid gap-6 px-5 py-3.5 mb-4.5">
        <div className="flex items-center gap-2.5">
          <img src={toAbsoluteUrl(`/media/images/600x600/${logo}`)} className="rounded-full size-10" alt="" />

          <div className="grid grid-cols-1 gap-0.5">
            <a href="#" className="text-gray-900 hover:text-primary-active text-md font-semibold mb-px">
              {title}
            </a>
