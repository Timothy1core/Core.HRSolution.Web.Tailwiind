...
const ModalReportUser = forwardRef(({
  open,
  onOpenChange
}, ref) => {
  const buildNotice = () => {
    return <div className="grid place-items-center gap-1 px-5">
          <div className="flex justify-center items-center rounded-full">
            <img src={toAbsoluteUrl('/media/avatars/300-1.png')} className="rounded-full max-h-[55px] max-w-full" alt="" />
          </div>
          <div className="flex items-center justify-center gap-1">
            <Link to="#" className="hover:text-primary-active text-2sm leading-5 font-semibold text-gray-900">
              Jenny Klabber
            </Link>

            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 15 16" fill="none" className="text-primary">
  ...
  const buildOptions = () => {
    ...
    return <div className="flex flex-col gap-5 px-5">
          <div className="text-2sm text-gray-900 font-semibold">
            Let us know why you’re reporing this person
          </div>
          <div className="flex flex-col gap-3.5">
            {items.map((item, index) => <label key={index} className="form-label flex items-center gap-2.5">
                <input className="radio radio-sm" name="report-option" type="radio" value="{{ loop.index }}" />
  ...
  const buildFooter = () => {
    return <div className="text-2sm font-medium text-center text-gray-800 p-x">
          Don't worry, your report is completely anonymous; the person you're <br />
          reporting will not be informed that you've submitted it
        </div>;
  };
  const buildButton = () => {
    return <div className="flex items-center gap-2.5 justify-end px-5">
