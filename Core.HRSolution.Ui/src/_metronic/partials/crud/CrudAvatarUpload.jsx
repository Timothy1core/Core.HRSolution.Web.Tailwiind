...
import { ImageInput } from '@/_metronic/components/image-input';
import { useState } from 'react';
const CrudAvatarUpload = () => {
  const [avatar, setAvatar] = useState([{
    dataURL: toAbsoluteUrl(`/media/avatars/300-2.png`)
  }]);
  return <ImageInput value={avatar} onChange={selectedAvatar => setAvatar(selectedAvatar)}>
      {({
      onImageUpload
    }) => <div className="image-input size-16" onClick={onImageUpload}>
          <div className="btn btn-icon btn-icon-xs btn-light shadow-default absolute z-1 size-5 -top-0.5 -end-0.5 rounded-full" onClick={e => {
        e.stopPropagation();
        setAvatar([]);
      }}>
            <KeenIcon icon="cross" />
          ...
          <div className="image-input-placeholder rounded-full border-2 border-success image-input-empty:border-gray-300" style={{
        backgroundImage: `url(${toAbsoluteUrl(`/media/avatars/blank.png`)})`
            ...
            <div className="flex items-center justify-center cursor-pointer h-5 left-0 right-0 bottom-0 bg-dark-clarity absolute">
              <svg xmlns="http://www.w3.org/2000/svg" width="14" height="12" viewBox="0 0 14 12" className="fill-light opacity-80">
