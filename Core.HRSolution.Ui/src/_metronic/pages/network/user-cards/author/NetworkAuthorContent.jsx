...
import { CardAuthor, CardAuthorRow } from '@/_metronic/partials/cards';
...
const NetworkAuthorContent = () => {
  ...
  const renderCard = (item, index) => <CardAuthor avatar={item.avatar} bgImage={item.bgImage} name={item.name} location={item.location} works={item.works} key={index} />;
  const renderListItem = (item, index) => <CardAuthorRow avatar={item.avatar} bgImage={item.bgImage} name={item.name} location={item.location} works={item.works} key={index} />;
  return <div className="flex flex-col items-stretch gap-5 lg:gap-7.5">
      <div className="flex flex-wrap items-center gap-5 justify-between">
        ...
        <div className="flex items-center flex-wrap gap-5">
          ...
          <div className="btn-tabs btn-tabs-sm" data-tabs="true">
            <a href="#" className={`btn btn-icon ${activeTab === 'cards' ? 'active' : ''}`} onClick={() => handleTabClick('cards')} data-tab-toggle="#author_cards">
              <KeenIcon icon="category" />
            </a>
            <a href="#" className={`btn btn-icon ${activeTab === 'list' ? 'active' : ''}`} onClick={() => handleTabClick('list')} data-tab-toggle="#author_list">
      ...
      {activeTab === 'cards' ? <div id="author_cards" className="flex flex-col gap-5 lg:gap-7.5">
        ...
        </div> : <div className="authors-row" id="author_list">
