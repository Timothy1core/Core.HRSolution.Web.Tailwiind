...
import { CardNFT2, CardNFT2Row } from '@/_metronic/partials/cards';
...
const NetworkNFTContent = () => {
  ...
  const renderItem = (item, index) => <CardNFT2 bgImage={item.bgImage} avatar={item.avatar} name={item.name} email={item.email} info={item.info} statistics={item.statistics} key={index} />;
  const renderRowItem = (item, index) => <CardNFT2Row bgImage={item.bgImage} avatar={item.avatar} name={item.name} email={item.email} info={item.info} statistics={item.statistics} key={index} />;
  return <div className="flex flex-col items-stretch gap-5 lg:gap-7.5">
      <div className="flex flex-wrap items-center gap-5 justify-between">
        ...
        <div className="flex items-center flex-wrap gap-5">
          ...
          <div className="btn-tabs btn-tabs-sm" data-tabs="true">
            <a href="#" className={`btn btn-icon ${activeTab === 'cards' ? 'active' : ''}`} onClick={() => handleTabClick('cards')} data-tab-toggle="#network_cards">
              <KeenIcon icon="category" />
            </a>
            <a href="#" className={`btn btn-icon ${activeTab === 'list' ? 'active' : ''}`} onClick={() => handleTabClick('list')} data-tab-toggle="#network_list">
      ...
      {activeTab === 'cards' ? <div id="network_cards">
