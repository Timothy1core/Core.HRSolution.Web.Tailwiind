...
const BlockList = ({
  ...
  const renderItem = (item, index) => {
    return <div key={index} className="flex items-center justify-between gap-2.5">
        <div className="flex items-center gap-2.5">
          <CommonAvatar image={item.avatar} imageClass="h-9 rounded-full" />

          <div className="flex flex-col gap-0.5">
            <Link to="/public-profile/teams" className="flex items-center gap-1.5 leading-none font-medium text-sm text-gray-900 hover:text-primary">
              {item.name}
            </Link>
