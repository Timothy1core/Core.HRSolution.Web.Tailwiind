...
import { DropdownCrud1 } from '@/_metronic/partials/dropdowns/general';
const CommunityBadges = () => {
  ...
  const renderItem = (item, index) => {
    return <div key={index} className="flex items-center justify-between flex-wrap group border border-gray-200 rounded-xl gap-2 px-3.5 py-2.5">
        <div className="flex items-center flex-wrap gap-2.5">
          <CommonHexagonBadge size="size-[50px]" badge={<KeenIcon icon={item.icon} className={`text-1.5xl ps-px ${item.iconColor}`} />} stroke={item.stroke} fill={item.fill} />
