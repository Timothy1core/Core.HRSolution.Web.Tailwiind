...
const MyBalance = ({
  ...
  return <div className={`card ${clsx(className && className)}`}>
      ...
      <div className="card-body flex flex-col gap-2 px-5 lg:px-7.5 pt-5">
        ...
        <div className="btn-group">
          {['Today', 'Week', 'Month', 'Year'].map(period => <button key={period} className={clsx('btn btn-sm btn-light flex justify-center w-1/4 tab-button focus:shadow-none hover:shadow-none btn-active:shadow-none btn-active:text-gray-900 btn-active:bg-gray-100', activePeriod === period && 'active')} onClick={() => setActivePeriod(period)}>
              {period}
            </button>)}
        </div>
      </div>

      <ApexChart id="my_balance_chart" options={options} series={options.series} type="area" max-width="361" height="250" className="px-3" />
    </div>;
};
export { MyBalance };