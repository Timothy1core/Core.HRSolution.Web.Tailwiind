...
const Integrations = () => {
  ...
  const renderItem = (item, index) => {
    return <div key={index} className="flex items-center justify-between flex-wrap border border-gray-200 dark:bg-secondary-clarity rounded-xl gap-2 p-3.5">
        <div className="flex items-center flex-wrap gap-3.5">
          <img src={toAbsoluteUrl(`/media/brand-logos/${item.logo}`)} className="size-8 shrink-0" alt="" />
        ...
        <div className="flex items-center gap-2 lg:gap-5">
          {item.checkbox ? <label className="switch switch-sm">
              <input type="checkbox" defaultChecked value="1" readOnly />
            </label> : <label className="switch switch-sm">
