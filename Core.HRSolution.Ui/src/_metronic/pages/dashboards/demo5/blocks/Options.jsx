...
const Options = () => {
  ...
  const renderItem = (item, index) => {
    return <div key={index} className="card px-5 lg:px-7.5 h-full bg-[length:85%] [background-position:7.5rem_-3.5rem] bg-no-repeat channel-stats-bg">
        <div className="flex flex-col gap-4 pt-6">
          <KeenIcon icon={item.icon} className="text-2xl text-gray-600" />

          <div className="flex flex-col gap-2.5 mb-2">
            <Link to={`${item.path}`} className="text-base font-semibold leading-none text-gray-900 hover:text-primary-active">
              {item.title}
            </Link>
