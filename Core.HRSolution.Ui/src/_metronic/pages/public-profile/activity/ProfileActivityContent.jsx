...
import { ActivitiesAnniversary, ActivitiesBloggingConference, ActivitiesDesignerWelcome, ActivitiesFollowersMilestone, ActivitiesInterview, ActivitiesNewArticle, ActivitiesNewTeam, ActivitiesPhotographyWorkshop, ActivitiesProductWebinar, ActivitiesProjectStatus, ActivitiesUpcomingContent, ActivitiesVirtualTeam } from '@/_metronic/partials/activities/items';
const ProfileActivityContent = () => {
  const [currentYear, setCurrentYear] = useState(2024);
  const years = Array.from({
    length: 8
  }, (_, i) => 2024 - i);
  return <div className="flex gap-5 lg:gap-7.5">
      {years.map((year, index) => <div key={index} className={`card grow ${year === currentYear ? '' : 'hidden'}`} id={`activity_${year}`}>
          ...
          <div className="card-body">
            ...
            {(year === 2024 || year === 2018) && <ActivitiesBloggingConference image={<Fragment>
      ...
      <div className="flex flex-col gap-2.5" data-tabs="true">
        {years.map((year, index) => <a key={index} href="#" data-tab-toggle={`#activity_${year}`} className={`btn btn-sm text-gray-600 hover:text-primary tab-active:bg-primary-light tab-active:text-primary ${year === currentYear ? 'active' : ''}`} onClick={() => {
        setCurrentYear(year);
      }}>
            {year}
          </a>)}
