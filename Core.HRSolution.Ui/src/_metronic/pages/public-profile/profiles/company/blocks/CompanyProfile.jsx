...
const CompanyProfile = () => {
  ...
  const renderRows = (row, index) => {
    return <div key={index} className="flex items-center gap-2.5">
        ...
        {row.info ? <a href={row.text} className="link text-sm font-medium">
            {row.text}
          </a> : <span className="text-sm text-gray-900">{row.text}</span>}
      </div>;
  };
  const renderProducts = (product, index) => {
    return <span key={index} className="badge badge-outline">
        {product.label}
      </span>;
  };
