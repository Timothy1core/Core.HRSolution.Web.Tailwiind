  ...
  const renderItem = (item, index) => {
    return <div key={index} className="flex items-center gap-2">
        <div className="flex items-center grow gap-2.5">
          <img src={toAbsoluteUrl(`/media/avatars/${item.avatar}`)} className="rounded-full size-9 shrink-0" alt="" />
        ...
        {item.connected ? <button className="btn btn-xs btn-icon btn-primary btn-outline rounded-full active">
            <KeenIcon icon="check" />
          </button> : <button className="btn btn-xs btn-icon btn-primary btn-outline rounded-full">
            <KeenIcon icon="plus" />
          </button>}
      </div>;
  };
