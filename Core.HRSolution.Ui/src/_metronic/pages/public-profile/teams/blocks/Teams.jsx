...
import { CardTeam, CardTeamRow } from '@/_metronic/partials/cards';
const Teams = () => {
  ...
  const renderItem = (item, index) => {
    return <CardTeam icon={item.icon} title={item.title} description={item.description} labels={item.labels} team={item.team} connected={item.connected} rating={item.rating} key={index} />;
  };
  const renderData = (data, index) => {
    return <CardTeamRow icon={data.icon} title={data.title} description={data.description} labels={data.labels} team={data.team} connected={data.connected} rating={data.rating} key={index} />;
  };
  return <div className="flex flex-col items-stretch gap-5 lg:gap-7.5">
      <div className="flex flex-wrap items-center gap-5 justify-between">
        ...
        <div className="btn-tabs" data-tabs="true">
          <a href="#" className={`btn btn-icon btn-sm ${activeView === 'cards' ? 'active' : ''}`} data-tab-toggle="#teams_cards" onClick={() => {
          setActiveView('cards');
        }}>
            <KeenIcon icon="category" />
          </a>
          <a href="#" className={`btn btn-icon btn-sm ${activeView === 'list' ? 'active' : ''}`} data-tab-toggle="#teams_list" onClick={() => {
          setActiveView('list');
        }}>
      ...
      {activeView === 'cards' && <div id="teams_cards">
      ...
      {activeView === 'list' && <div id="teams_list">
