...
import { CardCampaign, CardCampaignRow } from '@/_metronic/partials/cards';
const CampaignsContent = ({
  ...
  const renderProject = (item, index) => {
    return <CardCampaign logo={item.logo} logoSize={item.logoSize} title={item.title} description={item.description} status={item.status} statistics={item.statistics} progress={item.progress} url="#" key={index} />;
  };
  const renderItem = (data, index) => {
    return <CardCampaignRow logo={data.logo} logoSize={data.logoSize} title={data.title} description={data.description} status={data.status} statistics={data.statistics} url="#" key={index} />;
  };
      ...
      <div className="flex flex-wrap items-center gap-5 justify-between">
        ...
        <div className="flex gap-5">
          <div className="btn-tabs" data-tabs="true">
            <a href="#" className={`btn btn-icon ${currentMode === 'card' ? 'active' : ''}`} data-tab-toggle="#campaigns_cards" onClick={() => {
            setCurrentMode('cards');
          }}>
              <KeenIcon icon="category" />
            </a>
            <a href="#" className={`btn btn-icon ${currentMode === 'list' ? 'active' : ''}`} data-tab-toggle="#campaigns_list" onClick={() => {
            setCurrentMode('list');
          }}>
