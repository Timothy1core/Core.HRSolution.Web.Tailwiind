...
import { CardWork, CardWorkRow } from '@/_metronic/partials/cards';
...
const Works = () => {
  ...
  const renderItem = (item, index) => {
    return <CardWork title={item.title} image={item.image} authorName={item.authorName} authorAvatar={item.authorAvatar} likes={item.likes} comments={item.comments} key={index} />;
  };
  const renderData = (data, index) => {
    return <CardWorkRow description={data.description} title={data.title} image={data.image} authorName={data.authorName} authorAvatar={data.authorAvatar} likes={data.likes} comments={data.comments} key={index} />;
  };
  return <div className="flex flex-col items-stretch gap-5 lg:gap-7.5">
      <div className="flex flex-wrap items-center gap-5 justify-between">
        ...
        <div className="btn-tabs" data-tabs="true">
          <a href="#" className={`btn btn-icon ${activeView === 'cards' ? 'active' : ''}`} data-tab-toggle="#works_cards" onClick={() => {
          setActiveView('cards');
        }}>
            <KeenIcon icon="category" />
          </a>
          <a href="#" className={`btn btn-icon ${activeView === 'list' ? 'active' : ''}`} data-tab-toggle="#works_list" onClick={() => {
          setActiveView('list');
        }}>
      ...
      {activeView === 'cards' && <div id="works_cards">
          ...
          return renderItem(item, index);
            ...
            <Offer path="#" size="size-[90px]" iconSize="text-[2.25rem]" title="Offer a Work" subTitle="Ignite Professional Adventures" />
          </div>
      ...
      {activeView === 'list' && <div id="works_list">
          <div className="flex flex-col gap-5 lg:gap-7.5">
            {items.map((data, index) => {
          return renderData(data, index);
        })}
            <OfferRow path="#" size="size-[70px]" iconSize="text-[1.875rem]" title="Offer a Work" subTitle="Ignite Professional Adventures" />
          </div>
          <div className="flex grow justify-center pt-5 lg:pt-7.5">
            <a href="#" className="btn btn-link">
              Show more works
