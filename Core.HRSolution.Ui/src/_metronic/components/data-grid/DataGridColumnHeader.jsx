...
import { Button } from '@/_metronic/components/ui/button';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger } from '@/_metronic/components/ui/dropdown-menu';
  ...
  if (!filter && !column.getCanHide() && column.getCanSort()) {
    return <Button variant="ghost" size="sm" className={cn('-ms-3 h-8 data-[state=open]:bg-accent !ring-0 !ring-offset-0', className)} onClick={() => {
      ...
      if (isSorted === 'asc') {
        ...
        <span>{title}</span>
        {column.getIsSorted() === 'desc' ? <ArrowDown className="!size-[0.825rem]" /> : column.getIsSorted() === 'asc' ? <ArrowUp className="!size-[0.825rem]" /> : <ChevronsUpDown className="!size-[0.825rem]" />}
      </Button>;
  }
  return <div className={cn('flex items-center space-x-2', className)}>
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" size="sm" className={cn('-ms-3 h-8 data-[state=open]:bg-accent !ring-0 !ring-offset-0', className)}>
        ...
        <DropdownMenuContent align="start">
          ...
          {column.getCanSort() && <>
              <DropdownMenuItem onClick={() => column.toggleSorting(false)}>
                ...
                {column.getIsSorted() === 'asc' && <Check className="size-4 text-muted-foreground/90" />}
              </DropdownMenuItem>
              <DropdownMenuItem onClick={() => column.toggleSorting(true)}>
                <ArrowDown className="!size-[0.825rem] text-muted-foreground/90" />
                <span className="grow">Desc</span>
                {column.getIsSorted() === 'desc' && <Check className="size-4 text-muted-foreground/90" />}
              </DropdownMenuItem>
            </>}
