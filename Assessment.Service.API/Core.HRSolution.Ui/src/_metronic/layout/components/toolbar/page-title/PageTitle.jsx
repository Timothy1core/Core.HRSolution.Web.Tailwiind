  ...
  return (
    ...
    >
      ...
      {config.app?.pageTitle?.display && pageTitle && (
        ...
        >
          {pageTitle}
          {pageDescription && config.app?.pageTitle?.description && (
            <span
      ...
      {pageBreadcrumbs && pageBreadcrumbs.length > 0 && config.app?.pageTitle?.breadCrumb && (
        <>
          {config.app?.pageTitle?.direction === 'row' && (
          ...
          <ul className='breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0'>
            {pageBreadcrumbs.map((item, index) => (
              ...
              >
                {!item.isSeparator ? (
                  <Link className='text-muted text-hover-danger' to={item.path}>
                    {item.title}
                  </Link>
                ) : (
