...
import { defaultCreateAppData } from './IAppModels';
...
const CreateAppModal = ({ show, handleClose } = {}) => {
  ...
  const loadStepper = () => {
    if (stepperRef.current) {
      setStepper(StepperComponent.createInstance(stepperRef.current));
    }
  };
  ...
  const checkAppBasic = () => {
    return data.appBasic.appName && data.appBasic.appType;
  };

  const checkAppDataBase = () => {
    return data.appDatabase.databaseName && data.appDatabase.databaseSolution;
  };
  ...
  const nextStep = () => {
    setHasError(false);
    if (stepper) {
      if (stepper.getCurrentStepIndex() === 1 && !checkAppBasic()) {
        setHasError(true);
        return;
      }
      if (stepper.getCurrentStepIndex() === 3 && !checkAppDataBase()) {
        setHasError(true);
        return;
      }
      stepper.goNext();
