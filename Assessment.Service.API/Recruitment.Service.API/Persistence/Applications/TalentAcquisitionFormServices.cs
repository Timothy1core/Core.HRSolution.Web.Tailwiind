...
namespace Recruitment.Service.API.Persistence.Applications
{
	...
	) : ITalentAcquisitionFormServices
	{
		...
		public async Task<JsonResult> CreateTalentAcquisitionForm(TalentAcquisitionFormRequestDto tafRequest,string loggedEmployee)
		{
			...
			try
			{
				var taf = new TalentAcquisitionForm()
				{
				  ...
				  StatusId = 1,//set default value (pending)
		...
		public async Task<DataTableResponseDto<TalentAcquisitionFormDashboardDto>> RetrieveTalentAcquisitionFormDashboard(
			int group,
			int client,
			int status,
			int reason,
			...
			string sortDirection)
		{
			try
			{
				...
				var dataTableResult =  DataTableComponent<TalentAcquisitionFormDashboardDto>.Process(
					taf,
					search,
					start,
					length,
			...
			catch (Exception e)
			{
				...
				return new DataTableResponseDto<TalentAcquisitionFormDashboardDto>
				{
					Draw = draw,
					RecordsTotal = 0,
					RecordsFiltered = 0,
