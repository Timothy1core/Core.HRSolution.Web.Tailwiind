...
{
	public class CandidateRepository(HrisDbContext context) : ICandidateRepository
	{
		...
		int departmentGroupId, int departmentId, int jobId, int sourceId, bool isDisqualified)
		{
			...
			var candidates = await query
				.Select(s => new CandidateDto
				{
					...
					ClientGroupId = s.Job.Department.DepartmentGroupId,
					SourceName = s.Source.SourceName,
					StageName = s.ApplicationStatus.ProcessName,
					Recruiter = s.RecruiterInformation.FirstName +" "+ s.RecruiterInformation.LastName,
				})
				.ToListAsync();
